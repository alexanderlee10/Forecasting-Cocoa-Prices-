...
```{r}
df_monthly <- df_blue_monthly %>%
  left_join(filtered_inflation, by = "date") %>%
  left_join(df_gsci_monthly, by = "date")
```
...
```{r}
start_year  <- year(df_monthly$date[1])
start_month <- month(df_monthly$date[1])


ts_price <- ts(
  data = df_monthly$monthly_avg_price,
  start = c(start_year, start_month),
  frequency = 12
)

ts_inflation <- ts(
  data = df_monthly$inflation,
  start = c(start_year, start_month),
  frequency = 12
)

ts_gsci_div2 <- ts(
  data = df_monthly$gsci_div2,
  start = c(start_year, start_month),
  frequency = 12
)
...
# Actual
actual_vals <- df_monthly$monthly_avg_price
```
...
plot(
  x    = df_monthly$date,
  y    = df_monthly$monthly_avg_price,
  type = "l",
  col  = "blue",
  xlab = "Date",
  ylab = "Price",
...
plot(
  ...
  xlab = "Date",
  ylab = "Price",
  main = "Actual vs Model (only prices)"
)
...
# MAE and MSE
mae <- mean(abs(actual_values - fitted_values), na.rm = TRUE)
mse <- mean((actual_values - fitted_values)^2, na.rm = TRUE)
# print MAE and MSE
cat("MAE =", mae, "\n")
cat("MSE =", mse, "\n")
```
...
Train <- df_monthly_cleaned %>%
  filter(date < ymd("2023-12-01"))
Test <- df_monthly_cleaned %>%
  filter(date >= ymd("2023-12-01"))

xreg = Train[, c("inflation", "gsci_div2")]
Arima.fit <- auto.arima(Train$monthly_avg_price, xreg = xreg, seasonal = TRUE)
```
