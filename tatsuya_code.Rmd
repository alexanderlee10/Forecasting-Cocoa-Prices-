...
# Data from 2023 or before is for training
...
ts_gsci <- ts(df_gsci_monthly$monthly_gsci,
              frequency=12, start=c(2015,1))
ts_cocoa<- ts(df_sorted$monthly_price,
              frequency=12, start=c(2015,1))
...
# ccf_infl_cocoa$lag[which.max(ccf_infl_cocoa$acf)] gives the lag of max correlation
...
```{r}
# Shift 'date'
filtered_inflation_ <- filtered_inflation %>%
  mutate(date = date %m+% months(17))  # 17 months shift

filtered_sp_ <- filtered_sp %>%
  mutate(date = date %m+% months(25))  # 25 months shift

# Standardize the 'inflation' and 'gsci' columns so they have mean = 0 and sd = 1.
filtered_inflation_ <- filtered_inflation_ %>%
  mutate(inflation = scale(as.numeric(inflation)))

filtered_sp_ <- filtered_sp_ %>%
...
df_monthly_train <- train_prices %>%
  left_join(filtered_inflation_, by = "date") %>%
...
df_monthly_train <- train_prices %>%
  inner_join(filtered_inflation_, by = "date") %>%
  inner_join(train_gsci, by = "date")

df_monthly_train <- df_monthly_train %>%
  mutate(log_price = log(monthly_price))
```
...
# Neural Network Model (2nd model)
```{r}
# Set up
# This is the same with the set up above but didn't manipulate months (Deep Learning will automatically shift)
...
train_xreg <- cbind(ts_inflation, ts_gsci)
...
test_xreg <- cbind(ts_inflation_test, ts_gsci_test)
