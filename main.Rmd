...
```{r}
# Add a Year column (rows 1-30 represent years)
log_prices_long <- log_prices %>%
  mutate(Year = 1:n()) %>%  # Assign year numbers 30
  pivot_longer(
    cols = Jan:Dec,
    names_to = "Month",
    values_to = "Log_Price"
...
log_prices_long <- log_prices_long %>%
  mutate(
    Time = (Year - 1) * 12 + match(Month, month.abb)  # Time = 1, 2, ..., 300 (25 years * 12 months)
  )
```
